/*
 * ViClean Menu + Batterieanzeige
 *
 * Basiert auf viclean_menu, erweitert um Batterie-Ladungsanzeige.
 * Nutzt den Spannungsteiler am GPIO 34 (ADC_PIN) des LilyGO T-Display v1.1
 * mit GPIO 14 (ADC_EN) zur Aktivierung der Messung.
 *
 ...
 *
 * Batterie:
 *   GPIO 34 (ADC_PIN) - Batteriespannung ueber 1:2 Spannungsteiler
 *   GPIO 14 (ADC_EN)  - Spannungsteiler aktivieren (HIGH = an)
 *   eFuse-Kalibrierung wird automatisch genutzt falls vorhanden
 *   20-fach Sampling mit 25%-Quantil-Filter gegen ADC-Spikes
 *   LiPo-Entladekurve als Lookup-Table fuer praezise Prozentanzeige
 *
 * Zustandsmaschine:
...
// === Batterie Pins ===
#define ADC_EN   14   // Spannungsteiler aktivieren (HIGH = Messung moeglich)
#define ADC_PIN  34   // Batteriespannung (ueber 1:2 Teiler, ADC1_CH6)
...
#define BATT_READ_INTERVAL_MS  30000  // Batterie alle 30s messen
#define BATT_NUM_SAMPLES       20     // Anzahl ADC-Samples pro Messung
...
// === Batterie State ===
static int vref = 1100;            // ADC Referenzspannung in mV
float batteryVoltage = 0.0;        // Letzte gemessene Spannung
int batteryPercent = -1;            // Letzte Prozentanzeige (-1 = noch nicht gemessen)
unsigned long lastBatteryRead = 0;  // Zeitpunkt der letzten Messung
...
void batteryMonitorInit() {
  ...
  if (val_type == ESP_ADC_CAL_VAL_EFUSE_VREF) {
    vref = adc_chars.vref;
    Serial.printf("eFuse Vref Kalibrierung: %d mV\n", vref);
  } else {
    Serial.println("Standard Vref: 1100 mV (keine eFuse-Kalibrierung)");
  }
}
...
float readBatteryVoltage() {
  ...
  // 20 Samples nehmen und per Quantil-Filter Spikes entfernen
  float samples[BATT_NUM_SAMPLES];
  for (int i = 0; i < BATT_NUM_SAMPLES; i++) {
    uint16_t raw = analogRead(ADC_PIN);
    // raw/4095 * 2.0 (Spannungsteiler) * 3.3V * (vref/1000)
    samples[i] = ((float)raw / 4095.0) * 2.0 * 3.3 * ((float)vref / 1000.0);
    delay(2);
  }

  // Sortieren fuer Quantil-Filter
  for (int i = 0; i < BATT_NUM_SAMPLES - 1; i++) {
    for (int j = i + 1; j < BATT_NUM_SAMPLES; j++) {
  ...
  // 25. Perzentil: filtert positive ADC-Spikes zuverlaessig
  return samples[BATT_NUM_SAMPLES / 4];
}

int voltageToPercent(float voltage) {
  // LiPo-Entladekurve Lookup-Table (typisch fuer 3.7V LiPo)
  const float vTable[] =  { 4.20, 4.15, 4.11, 4.08, 4.02, 3.98, 3.95,
                             3.91, 3.87, 3.85, 3.84, 3.82, 3.80, 3.79,
                             3.77, 3.75, 3.73, 3.71, 3.69, 3.61, 3.27 };
  const int   pTable[] =  {  100,   95,   90,   85,   80,   75,   70,
                               65,   60,   55,   50,   45,   40,   35,
                               30,   25,   20,   15,   10,    5,    0 };
  const int sz = sizeof(vTable) / sizeof(vTable[0]);

  if (voltage >= vTable[0]) return 100;
  if (voltage <= vTable[sz - 1]) return 0;

  for (int i = 0; i < sz - 1; i++) {
    if (voltage >= vTable[i + 1]) {
...
// Batterie-Icon + Prozent zeichnen (TextSize 1, an Position x,y)
// Icon: 18x9 px Rahmen + 2x5 px Nub + Fuellstand + Prozenttext
void drawBatteryIndicator(int x, int y) {
  if (batteryPercent < 0) return;  // Noch nicht gemessen

  // Farbe nach Ladestand
  uint16_t fillColor;
  ...
  tft.fillRect(x, y, 52, 10, TFT_BLACK);

  // Batterie-Rahmen (18x9)
  tft.drawRect(x, y, 18, 9, TFT_DARKGREY);
  // Plus-Pol Nub rechts (2x5)
  tft.fillRect(x + 18, y + 2, 2, 5, TFT_DARKGREY);

  // Fuellstand (innen: 14x5 px maximal)
  int maxFill = 14;
  int fillWidth = (maxFill * batteryPercent) / 100;
  if (fillWidth > 0) {
    tft.fillRect(x + 2, y + 2, fillWidth, 5, fillColor);
  }

  // Prozent-Text rechts daneben
  tft.setTextSize(1);
  tft.setTextColor(fillColor, TFT_BLACK);
  tft.setCursor(x + 22, y + 1);
...
void goToSleep() {
  ...
  // ADC-Spannungsteiler deaktivieren fuer Stromsparen
...
// --- Statusleiste (gemeinsam fuer alle Screens) ---
// Layout: [ViClean] [BattIcon %%] [Zzz XXs] [CONN/----]
void drawStatusBar() {
  // Titel
  tft.setTextSize(1);
...
void drawSettingsListScreen() {
  ...
  // Batterie-Anzeige rechts oben
  drawBatteryIndicator(140, 3);
...
void updateSleepTimer() {
  ...
  if (appState == STATE_MAIN) {
    tft.fillRect(150, 3, 55, 12, TFT_BLACK);
    tft.setTextSize(1);
    tft.setCursor(150, 5);
    tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
    tft.printf("Zzz %lus", remaining);
  } else if (appState == STATE_SETTINGS_LIST) {
    tft.fillRect(200, 1, 40, 12, TFT_BLACK);
    tft.setTextSize(1);
    tft.setCursor(200, 3);
    tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
    tft.printf("Zzz%lus", remaining);
...
// Batterie-Anzeige in Statusleiste aktualisieren (ohne volles Redraw)
void updateBatteryDisplay() {
  if (appState == STATE_SETTINGS_EDIT) return;

  if (appState == STATE_MAIN || appState == STATE_RUNNING) {
    drawBatteryIndicator(60, 4);
  } else if (appState == STATE_SETTINGS_LIST) {
    drawBatteryIndicator(140, 3);
  }
...
void setup() {
  ...
  Serial.println("\n=== ViClean Menu + Batterie ===");
  printWakeReason();

  // Einstellungen aus NVS laden
  loadSettings();

  // Batterie-ADC initialisieren
  batteryMonitorInit();

  // Erste Batteriemessung
...
void loop() {
  ...
  // Periodische Batteriemessung
  unsigned long now = millis();
  if (now - lastBatteryRead >= BATT_READ_INTERVAL_MS) {
    updateBatteryReading();
